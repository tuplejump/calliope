...
object CassandraSparkDataConvertor {
  ...
  private def readTypedValue(crow: CassandraRow, name: String, valueType: CassandraDataType): Any = {
    valueType.getName match {
      ...
      case CassandraDataType.Name.VARINT =>
        val varint: BigInteger = crow.getVarint(name)
        if (varint == null) {
          null
        } else {
          BigDecimal(varint) //Big Integer is treated as BigDecimal by Catalyst
        }
      ...
      case CassandraDataType.Name.UUID =>
        val guuid: UUID = crow.getUUID(name)
        if (guuid == null) null else guuid.toString //TODO: Stopgap solution. add datatype to SparkSQL
      case CassandraDataType.Name.TIMEUUID =>
        val tuuid: UUID = crow.getUUID(name)
        if (tuuid == null) tuuid else tuuid.toString //TODO: Stopgap solution. Should be struct.
      case CassandraDataType.Name.TIMESTAMP =>
        val date: Date = crow.getDate(name)
        if (date == null) null else new Timestamp(date.getTime)
  ...
  private[sql] def serializeValue(data: Any, dtype: SerCassandraDataType): ByteBuffer = {
    if (data == null) {
      ByteBuffer.allocate(0) //TODO: Find a better way to handle this later. Once we have native CQL support for writes
    } else {
